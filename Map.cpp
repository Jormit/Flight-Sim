...
Vertex Map::getVertex(const glm::vec2 position, float freq, int depth, int heightScale) {
    Vertex v;
    float h = heightScale * perlin2d(position.x, position.y, freq, depth);
    float hx = 100.f * (heightScale * perlin2d(position.x + 0.01f, position.y, freq, depth) - h);
    float hy = 100.f * (heightScale * perlin2d(position.x, position.y + 0.01f, freq, depth) - h);
